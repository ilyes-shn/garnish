...
const Accordion = ({title, body}) => {
  ...
  return (
    <div className="accordion">
      <div className="accordion_title" onClick={() => setShow(!show)}>
        <h3>{title}</h3>
        <img src="/assets/images/arrow.svg" alt="arrow" style={{ transform: show ? 'rotate(180deg)' : 'none'}}/>
      </div>
      {show && (
...
<h5>Apply by sending an email to office@garnishstudio.com</h5>
        </div>
      )}
    </div>
  );
